...
DUK_LOCAL duk_bool_t duk__push_safe_string_prop_summary(duk_context *ctx, duk_hobject *h, duk_small_uint_t stridx) {
	duk_propdesc desc;
	...
	duk__push_hstring_readable_unicode(ctx, duk_get_hstring(ctx, -1), 0 /*quotes*/);
	duk_remove(ctx, -2);
	return 1;
}
	...
	if (tv == NULL) {
		...
		case DUK_TAG_OBJECT: {
			...
			if (DUK_HOBJECT_IS_CALLABLE(h)) {
				...
				duk_push_string(ctx, "[Function ");
				if (!duk__push_safe_string_prop_summary(thr, h, DUK_STRIDX_NAME)) {
					goto generic_object;
				}
				duk_push_string(ctx, "]");
				duk_concat(ctx, 3);
			...
			if (duk_hobject_prototype_chain_contains(thr, h, thr->builtins[DUK_BIDX_ERROR_PROTOTYPE], 1 /*ignore_loop*/)) {
				...
				duk_push_string(ctx, " ");
				if (!duk__push_safe_string_prop_summary(thr, h, DUK_STRIDX_MESSAGE)) {
					goto generic_object;
				}
				duk_push_string(ctx, "]");
				duk_concat(ctx, 5);
			...
			if (DUK_HOBJECT_GET_CLASS_NUMBER(h) == DUK_HOBJECT_CLASS_DATE) {
				/* FIXME; date _Value or ISO 8601 UTC? */
			}
		 ...
		 generic_object:
			...
			/* Default generic summary is based on class number. */
			duk_set_top(ctx, entry_top);
			duk_push_string(ctx, "[");
			duk_push_class_string_tval(ctx, tv);
			duk_push_string(ctx, "]");
			duk_concat(ctx, 3);  /* FIXME: direct sprintf */
			break;
		}
		case DUK_TAG_BUFFER: {
